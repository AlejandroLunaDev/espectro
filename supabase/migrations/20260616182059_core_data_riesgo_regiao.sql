-- Core data tables for the CDRView dataset (Visent/Wongola) and the materialized
-- digital-exclusion risk score per cluster. See ARQUITECTURA_DATOS_IA.md (issue #12)
-- and the CDRView Technical Reference v2 for the source schema/business rules.
...
create table if not exists antenas_flp (
  ecgi       text primary key,
  cluster    text not null references clusters (cluster),
  municipio  text not null,
  lat        double precision not null,
  lon        double precision not null
);
...
create table if not exists assinantes (
  ...
  home_cluster      text not null references clusters (cluster),
...
create index if not exists idx_assinantes_home_cluster on assinantes (home_cluster);

-- Ya viene agregado por (antena, dia, periodo) en el dataset fuente: se carga tal cual.
create table if not exists tensor_concentracao (
...
create index if not exists idx_tensor_concentracao_cluster on tensor_concentracao (cluster);
create index if not exists idx_tensor_concentracao_periodo on tensor_concentracao (periodo);

-- Tabla estrella: calculada una vez por el pipeline de ingestion (data-pipeline/),
-- GET /mapa la lee directo. Los 3 componentes se guardan separados para que la IA
-- pueda explicar el "por que" sin recalcular (ver ARQUITECTURA_DATOS_IA.md seccion 5).
create table if not exists riesgo_regiao (
  cluster            text primary key references clusters (cluster),
  municipio          text not null,
  lat                double precision not null,
  lon                double precision not null,
...
create index if not exists idx_riesgo_regiao_score on riesgo_regiao (score_riesgo desc);

-- Referencia de los 27 clusters geograficos (CDRView Technical Reference v2, seccion 12).
insert into clusters (cluster, municipio, lat, lon, perfil) values
